...
void mgHttpClient::_httpevent_handler(struct mg_connection *c, int ev, void *ev_data, void *user_data)
{
    ...
    case MG_EV_HTTP_MSG:
    {
        ...
        // apply html filter to buffer
        if (client->_use_html_filter && client->_buffer_len>0) {
            client->_html_filter.reset_state();
            client->_buffer_len =  client->_html_filter.filter_chunk(client->_buffer, client->_buffer_len);
        }
...
bool mgHttpClient::set_html_filter(const char *filter)
{
    _use_html_filter = _html_filter.set_filter(filter);
    return true;
}

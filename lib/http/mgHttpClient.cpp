...
void mgHttpClient::_httpevent_handler(struct mg_connection *c, int ev, void *ev_data, void *user_data)
{
    ...
    case MG_EV_HTTP_MSG:
    {
        ...
        if (client->_buffer_len>0 && client->_apply_html_filter) {
#ifdef VERBOSE_HTTP
            Debug_printf("mgHttpClient: applying html filters to skip:");
#endif
            // Applies simple forward-only in-place filtering of html tags,
            // adjusting _buffer_len at the end.
            
            HtmlFilterMode mode = FILTER_MODE_TEXT;
            int buffer_len = client->_buffer_len;
            int dest_index=0; // Index to write current char. We move this back when skipping
            int start; // index of start of tag, etc
            int stop1='<',stop2=-1; // Stop characters
            std::string skip_tag; // Current tag being skipped
            std::string tag; // Placeholder for tag
            std::string attr; // Placeholder for attribute
            bool skip_char=false; // Index to start skipping
            bool self_closing_tag = false; // true if the tag is self closing r.g. <br/>
            ...
            for(int i=0;i< buffer_len;i++) {
                ...
                switch (mode) {
                    case FILTER_MODE_TEXT:
                        mode = FILTER_MODE_TAG_NAME;
                        stop1=' '; stop2='>';
                        start = i + 1;
                        break;
                    case FILTER_MODE_TAG_NAME:
                        tag = std::string(&client->_buffer[start], i-start);
                        util_string_toupper(tag);
                        if(tag=="CTYPE PE") {
                            tag="!DOCTYPE";
                        }  else if (util_starts_with(tag, "!--")) {
                            tag="!--";
                        }
                        
                        in_pre = tag=="PRE";
                        if (skip_tag.length()>0) {
                            if (skip_tag == tag) {
                                skip_tag = "";
                                skip_char=false;
                                if (c==' ') {
                                    // Skip ahead until a > is found, covers odd cases like </div >
                                    while (i<buffer_len && client->_buffer[i] != '>') {i++;}
                                }
                            } else {
                                 // Since we are skipping until we find the right end tag
                                 // skip past all attributes on this tag
                                if (c==' ') {
                                    while (i<buffer_len && client->_buffer[i] != '>') {i++;}
                           ...
                           if (client->_html_filter_tags_disallowed.count(tag) || (
                                client->_html_filter_tags_allowed.size()>0 && 
                                client->_html_filter_tags_allowed.count(tag) == 0 )
                           ) {
                                // Roll back dest index to the start of the tag
                                dest_index-=(i-start+2);

                                // If this is a comments tag, skip ahead until -->
                                if (tag=="!--") {
                                    while (
                                        i<buffer_len && (
                                            client->_buffer[i] != '>' ||
                                ...
                                // Else if we stopped on a space instead of >, skip ahead to find >
                                else if (c==' ') {
                                    while (i<buffer_len && client->_buffer[i] != '>') {i++;}
                                }
                                
                                // Check if this is a self closing tag
                                if (!self_closing_tag && i<buffer_len && client->_buffer[i-1] == '/') {
                                    self_closing_tag=true;
                                } else if (tag=="!DOCTYPE" || tag=="META" || tag=="LINK" || tag=="IMG" || tag=="BR" || tag=="HTML" || tag=="/HTML" || tag=="!--") {
                                    self_closing_tag=true;
                                }

                                // If the tag is not self closing, we will skip until the closing
                                // tag is found
                                if (!self_closing_tag) {
                                    skip_tag=std::string("/");
                                    skip_tag.append(tag);                            
                        ...
                        // If not skipping, so let's proceed to find attributes if not
                        // at the end
                        if ( mode == FILTER_MODE_TAG_NAME && filter_attributes &&  c==' ') {
                            mode = FILTER_MODE_ATTR_NAME;
                            stop1='='; stop2='>';
                            start = i + 1;
                        } else {
                            mode = FILTER_MODE_TEXT;
                    ...
                    case FILTER_MODE_ATTR_NAME:
                        if (c=='=' && i > start + 1) {
                            attr = std::string(&client->_buffer[start], i-start);
                            util_string_trim(attr);
                            util_string_toupper(attr);
                            
                            // Should we skip the attribute?
                            if (client->_html_filter_attrs_disallowed.count(attr) || (
                                client->_html_filter_attrs_allowed.size()>0 && 
                                client->_html_filter_attrs_allowed.count(attr) == 0 )
                            ) {
...
bool mgHttpClient::set_html_filter(const char *html_filter)
{
     
    #ifdef VERBOSE_HTTP
        Debug_printf("mgHttpClient: Reading html filters: ");
    #endif
    ...
    std::istringstream filters(html_filter);
    std::string filter;
    ...
    _apply_html_filter = filter_count;
        
  
    #ifdef VERBOSE_HTTP
        Debug_printf("\nmgHttpClient: html filter count is %i\n",filter_count);
    #endif

    // return ESP_OK == esp_http_client_set_html_filter(_handle, html_filter);
    return true;
}
